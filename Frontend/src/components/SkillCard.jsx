...
const SkillCard = () => {
    ...
    return (
        <Card className={`group cursor-pointer transition-all duration-300 hover:shadow-xl hover:-translate-y-1 bg-card border-2 border-border/40 hover:border-(--primary-gradient-start)/30 relative overflow-hidden ${isCompact ? 'p-4' : 'p-6'
            }`}
        >
                ...
                <div className="relative z-10">
                        ...
                        <div className="flex items-center space-x-3">
                            ...
                            <div>
                                <div className="flex items-center space-x-2">
                                    <h3 className={`font-semibold group-hover:text-(--primary-gradient-start) transition-colors duration-200 ${isCompact ? 'text-sm' : 'text-base'
                                        }`}>
                                        {provider.name}
                                    </h3>
                                    {provider.isVerified && (
                                        <CheckCircle className={`text-(--primary-gradient-start) ${isCompact ? 'w-3 h-3' : 'w-4 h-4'
                                            }`} />
                                    )}
                                </div>
                        ...
                        <div className="flex flex-wrap gap-1.5">
                            {provider.skills.slice(0, isCompact ? 2 : 3).map((skill) => (
                                <Badge
                                    key={skill}
                                    variant="outline"
                                    className={`bg-background/50 border-border/40 hover:border-(--primary-gradient-start)/40 transition-colors duration-200 ${isCompact ? 'text-xs px-2 py-0.5' : 'text-xs'
                                        }`}
                                >
                                    {skill}
                                </Badge>
                    ...
                    <div className={`flex items-center justify-between ${isCompact ? 'mb-3' : 'mb-4'}`}>
                        ...
                        <div className="text-right">
                            <p className={`font-bold text-(--primary-gradient-start) ${isCompact ? 'text-sm' : 'text-base'
                                }`}>
