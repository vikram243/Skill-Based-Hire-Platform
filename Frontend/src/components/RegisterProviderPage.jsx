...
function RegisterProviderPage({ onNavigate, onBack }) {
  ...
  const removeCustomSkill = (skillName) => {
    setFormData(prev => ({
      ...prev,
      customSkills: prev.customSkills.filter(s => s !== skillName)
    }));
  };
  ...
  const setPricing = (skill, price) => {
    setFormData(prev => ({
      ...prev,
      pricing: { ...prev.pricing, [skill]: price }
    }));
  };
                    ...
                    <div
                      ...
                      className={`p-3 border border-border rounded-lg cursor-pointer transition-colors ${
                        formData.selectedSkills.includes(skill.name)
                          ? 'border-(--primary-gradient-start) bg-(--primary-gradient-start)/5'
                          : 'border-border hover:border-(--primary-gradient-start)/50'
                      }`}
                ...
                {allSelectedSkills.length > 0 ? (
                  <div className="flex flex-wrap gap-2">
                    {allSelectedSkills.map((skill) => (
                      <Badge key={skill} className="bg-(--primary-gradient-start) text-white">
                        {skill}
                      </Badge>
                    ))}
                  </div>
              ...
              <div className="space-y-4">
                <div className="flex items-start gap-3">
                  ...
                  <label htmlFor="terms" className="text-sm leading-relaxed">
                    I agree to the <a href="#" className="text-(--primary-gradient-start) underline">Terms of Service</a> and 
                    <a href="#" className="text-(--primary-gradient-start) underline"> Privacy Policy</a>
                  </label>
                </div>
            ...
            <CardContent className="p-8">
              ...
              <div className="space-y-3">
                <Button 
                  ...
                  className="w-full bg-linear-to-r from-(--primary-gradient-start) to-(--primary-gradient-end) text-white"
                  size="lg"
                >
                  Go to Profile
                </Button>
